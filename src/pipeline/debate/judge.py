...
class Judge:
    ...
    def __init__(self, model_config: Dict[str, Any], llm_client):
        """
        Args:
    ...
    def _get_model_signal_text(self, model_verdict: str, model_confidence: float) -> str:
        """Generate SLM-guided signal text for Judge."""
        if not model_verdict or model_confidence is None:
            return ""
        
        conf_pct = f"{model_confidence:.0%}"
        
        if model_confidence >= 0.90:
            return f"""CLASSIFIER SIGNAL: A specialized model strongly suggests {model_verdict} ({conf_pct}).
Treat as strong hypothesis - align if debate supports it."""
        elif model_confidence >= 0.70:
            return f"""CLASSIFIER SIGNAL: A model suggests {model_verdict} ({conf_pct}).
Consider as one additional vote, but debate arguments take priority."""
        else:
            return f"""CLASSIFIER SIGNAL: A model is uncertain, leaning {model_verdict} ({conf_pct}).
This case needs careful reasoning - rely primarily on debate quality."""
    ...
    def _build_judge_prompt(
        ...
        # ===== BUILD PROMPT =====
        prompt = f"""**ROLE:** You are the JUDGE - Final Arbiter for Fact-Checking.

**CONTEXT:**
- Round 1: Debators analyzed INDEPENDENTLY
- Round 2: Debators cross-examined each other based on debate only
- Your job: Evaluate the QUALITY of reasoning and make FINAL verdict
...
"""
        ...
        # R2 verdicts (after seeing Model)
        prompt += "\n**ROUND 2 VERDICTS (After seeing Model prediction):**\n"
        final_round = debate_history[-1] if debate_history else []
        for arg in final_round:
            agent_name = self.AGENT_NAME_MAP.get(arg.role, arg.debator_name)
            short_reasoning = arg.reasoning[:200] + "..." if len(arg.reasoning) > 200 else arg.reasoning
            prompt += f"- {agent_name}: **{arg.verdict}** ({arg.confidence:.0%})\n"
            prompt += f"  Reasoning: {short_reasoning}\n"
        ...
        if r1_round and final_round:
            ...
            if flips:
                prompt += f"\n⚠️ **VERDICT CHANGES (R1→R2):** {', '.join(flips)}\n"
...
1. **CONSENSUS (3/3 agree):** Follow the consensus. Do NOT override.

2. **MAJORITY (2/1):** Follow majority verdict.
   - Only consider minority if they found a quote that majority MISSED.
   - Do NOT override just because minority "sounds more convincing".

3. **SPLIT DECISION (1-1-1):** (Rare case: Support vs Refute vs NEI)
   - Evaluate who has the STRONGEST evidence quote.
   - If evidence is weak/ambiguous -> **NEI** (Safety first).
   - **Best Argument Wins**: Choose the verdict with the most direct quote match.

**YOUR TASK:**
Summarize the debate and give your final verdict with structured explanation (XAI).
  ...
  - NEI: "Hiện tại, bằng chứng được cung cấp chưa đủ để kết luận tuyên bố đúng hay sai."""
    ...
    def _parse_judge_response(
        ...
        try:
            ...
            # Handle explanation_vi (new) or explanation (old)
